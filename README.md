...
| **scripts/python/download_cellranger_data.py** | scRNA-seq, snRNA-seq, scATAC-seq (10x)              | **~1–4 TB** (--minimal; scATAC is huge) / **multi-TB** (full 31) | x86 only (Cell Ranger) |
...
python scripts/python/download_cellranger_data.py --minimal   # 15 runs; ~1–4 TB FASTQs (5 scATAC)
# python scripts/python/download_cellranger_data.py           # 31 runs; multi-TB
...
python scripts/python/run_cellranger.py --ref-scrna data/refs/refdata-gex-GRCm39-2024-A \
...
│   └── mermaid/
│       ├── brd4_il1b_circuit.mmd         # BRD4→IL1B→NF-κB→MEOX1 diagram
│       └── brd4_il1b_circuit_oneliner.mmd
...
ARM64: `scripts/python/download_data.py` (bulk, ChIP, CUT&RUN). x86: `scripts/python/download_cellranger_data.py` + Cell Ranger (scRNA, scATAC).
